...
- **One rule needs a heai-tools exception.** Rung 2, "already in this codebase → reuse it, don't rewrite", and the review tag `yagni: ... inline it until a second one exists` are exactly what design principle 1 forbids across tools: a tool that needs another's logic depends on its package or copies it on purpose, never imports its source.
...
- **`ponytail:` markers and the debt ledger.** A deliberate shortcut with a known ceiling is marked in a comment - `# ponytail: global lock, per-account locks if throughput matters` - naming the ceiling and the upgrade trigger; `/ponytail-debt` harvests them into a ledger, one row per marker, and flags any without a trigger as `no-trigger`.
  For heai-tools this is a tracker feature that needs no model: a sweep that greps the markers, resolves each file to its territory through the map, and files a `backlog` task per marker through `tasks new`, keyed by a hash of the comment so a rerun files nothing twice.
