...
```
   ...
   reactor serve      ─── the one daemon: one event, one rule, one script ──────────────────────────────
     a tick, a webhook, a poll, an emit ──► an event ──► the rule that matches ──► one script
     the minute rule                                                            ──► flow settle: facts folded, clocks and guards fired
...
A map may govern several repositories; `repositories.<name>.localPath` in the map says where each checkout is, and `pod repo add <name>` clones from it.
...
```sh
architect template > architecture.yaml          # a starting map to edit
architect check [--strict]                      # the validator; 1 on a problem
architect owner src/api/routes.ts --json        # the territory and the actor for one path
architect territories --actor api-owner --json  # what one actor owns, with each owner's type
architect actors --json
architect context api-owner                     # the actor's and its territories' context, for a prompt
git diff --name-only main...branch | architect gate --actor api-owner --format json   # 1 on a boundary crossed
```
...
tasks new cache-the-index --title "Cache the index" --territory api --body -    # body from stdin
tasks set cache-the-index --status in-progress
tasks set cache-the-index --status blocked --note "Blocked by [t7](t7.md)."      # notes append; bodies are never rewritten
tasks list --status todo --json
tasks show cache-the-index --json | jq -r '.run // empty'                      # the task's run block, if it has one
tasks build --check                                                             # 1 when INDEX.md is stale
```
...
```sh
flow start review --link doc=spec-7 --by file.sh          # prints the id
flow advance <id> approved --by human
flow advance <id> checked --data '{"failed":0}' --by check.sh
flow link <id> branch=agent/x/t1 --waits-on <other-flow>
flow touch <id>                                   # the heartbeat an `after` counts from
flow list review --in open --json
flow show <id> --json                             # state, links, journal, actions
flow trace doc=spec-7                             # one timeline across every linked flow
flow stuck --older 1h                             # what nothing has moved or touched
flow settle                                       # fold inboxes, expire states, fire guards, rebuild views
flow check [--fix]; flow reindex [--repin]; flow definitions
```
...
`reactor.yaml` declares **sources** - `schedule` on a cron, `git` watching branches of a repository, `webhook` with a provider or a generic shape, `poll` over a URL, `cli` fed only by `reactor emit`, `dir` watching a drop directory, or a module path - and **rules**, each matching a source and optionally a kind and payload fields, and naming one command to `run` with the event in its environment.

```sh
reactor serve --port 4949         # the daemon: schedules, polls, the webhook receiver; under launchd or systemd
reactor tick                      # the same, one pass, from cron on a machine with no daemon
reactor emit tasks_cli task.todo --key t1 --payload '{"slug":"t1"}' --act
reactor status; reactor events --since 24h; reactor retry <event-id>
reactor test pr-merged --event fixtures/merged.json
```
...
`pod up --image <tag>` runs one persistent container from an image with a Herdr server inside; the clones live under `pod/repos`, one **worktree per piece of work** under `pod/worktrees`, each a Herdr workspace with Herdr's id.

```sh
pod build                                    # the base, then every image in pod.yaml
pod up --image proj/dev [--env-from ~/.heai/proj.env]
pod repo add app                             # a clone from the map's localPath, kept in sync by the host
ws=$(pod open app --branch agent/api-owner/t1 --actor api-owner)     # w2
pod start "$ws" --agent claude               # w2:p2; commits in it are attributed to the actor
pod prompt "$ws" --file prompt.md --notify /heai/flow/<id>/inbox --event exited
pod run "$ws" --file block.sh --notify /heai/flow/<id>/inbox --event exited
pod wait "$ws" --until blocked --notify /heai/flow/<id>/inbox --event needs-input
pod wait "$ws" --notify /heai/flow/<id>/inbox --every 60s          # a heartbeat, for a state with an `after`
pod repo pull-branch app agent/api-owner/t1  # the branch home, for the gate and the landing
pod attach "$ws"                             # a human, in Herdr, looking at the agent
pod close "$ws" --keep-worktree
```
...
The directory is a flow's inbox, mounted once in `pod.yaml`, and `flow settle` reads it; the pod never learns what a flow is.
...
An `htop`-shaped terminal view, read from the tools' files and CLIs, writing nothing of its own: every task with the actor it routes to and where its blocker stands; what `flow` says is open and stuck; the session flows and the workspaces `pod list` reports.
Keys act through the tools - `a` starts a session, `e` edits a task, `s` runs `reactor tick`, `c` runs the project's cancel script when it has one - and `operator --once` or `--json` is the same screen for a script or a CI job, exit `1` when anything is red.
...
A human files and triages tasks, moves one to `todo` when it is ready, and is the only one who may make a move a definition marks `by: [human]`: `flow advance <id> approved` is the approval, and nothing proceeds until it happens.
When an agent stops to ask, a human answers it with `pod prompt`, or steps in with `pod attach` and sees what the agent sees.
...
An agent works in a pod workspace on its own branch, prompted with `architect context <actor>` and the task, commits under its own name, and is judged by `architect gate --actor <actor>` on the diff it produced.
...
Any harness installed in the image is an agent kind: `pod start --agent claude`, `--agent codex`.
...
pod build && pod up --image myproject/dev && pod repo add <repo>
reactor serve                                                 # under the platform's service manager
operator
```

A template under `templates/` is the usual starting point for `flows/` and `scripts/`; the two team documents each say which.

**The loop, once it runs.** A human moves a task to `todo`. A rule, on the minute or on the `task.todo` emit, runs a script that finds the actor the task routes to and starts a flow. Entering its first state runs the script that opens a workspace and prompts the agent, with `--notify` into the flow's inbox. The agent works; its heartbeat touches the flow; when it settles, a fact lands and the next `flow settle` moves the flow on, and the next state's script brings the branch home, gates it, and records the verdict. From there the project's own definitions say what follows: a landing, a review, a publish, a human's approval.

**Where a human steps in.** At the map, when a boundary should move. At a task, when the work is not yet worth an agent's time or is routed to two actors. When an agent asked. At every `by: [human]` move. At `flow stuck` and the red on `operator`, when a script failed and left its log under the flow's `actions/`; the person reads it and runs the script or the move again.
...
**Reading what happened.** `flow trace <link>` is one timeline across every flow that shares the link, every line signed by who made the move. `reactor events` is what came in and what ran. `git log` on the project is every change to the process itself.
...
hooks:
  open:    { run: scripts/review/check.sh }       # run the checks, then `flow advance checked --data ...`
  checked: { run: scripts/review/notify.sh }      # file a task for the reviewer naming the flow
  stale:   { run: scripts/review/requeue.sh }
```

- **A hook is one script**, `run`, keyed by the state it fires on entering. A script that wants a task filed calls `tasks new`, one that wants a flow moved calls `flow advance` or writes a fact into an inbox, one that wants the reactor told calls `reactor emit`.
- **`flow` runs it once per entry**, detached, after its locks are released, so a script may `flow advance` its own flow without waiting. A flow that re-enters a state runs it again; a `link` or a `touch` enters no state.
- **It never advances the flow on its own.** The script does, because whether the move happened is the script's to say and its data is what the next `when` reads. A script that exits non-zero leaves the flow where it was, with its exit and log under `flow/flows/<id>/actions/`, and `flow stuck` shows the flow within the hour.
- **It runs on the host, in the project directory**, with `HEAI_FLOW`, `HEAI_DEFINITION`, `HEAI_STATE`, `HEAI_EVENT`, `HEAI_SEQ`, one `HEAI_LINK_<NAME>` per link, and `HEAI_LINE` naming a file holding the journal line as JSON. It names no paths and receives none; every tool finds its files in the directory by convention. A script that wants the container runs `pod run`; the host is where git, `gh` and credentials are.
- **Two scripts of one flow may overlap.** `flow` spawns and forgets. A script that must not overlap takes its own lock, with `mkdir`.
- **The block is validated and pinned** with the rest of the definition when a flow starts, so a definition edited while flows are open changes their scripts only on `flow reindex --repin`.
...
rules:
  - { name: minute,   on: { source: clock },                      run: "flow settle && scripts/pick.sh" }
...
The minute rule is what keeps flow moving: `flow settle` folds the facts the containers dropped, expires the states whose `after` elapsed and fires the guards, and each state entered runs its script from inside `flow`, with no event ever reaching the reactor.
A task moved to `todo` by a person reaches the reactor when the person, or the wrapper they use, runs `reactor emit tasks_cli task.todo`; without it the minute rule picks the task up within sixty seconds.
...
- **`templates/testing/`** - designed: a `suite` definition, `open → pass | fail`, started by a nightly rule, `run.sh` running the suite through `pod run`, and a hook on `fail` that files a task in the territory `architect owner` gives for each failing path.
...
- **Every move is a file someone can find.** A state's script leaves its exit and log beside its flow; a rule's run leaves them beside its event; a script's outcome is a journal line it signed; a task's move is a commit. Nothing is in memory, and `reactor serve` can die at any line.
...
3. **Repinning.** A definition edited while flows are open keeps the old scripts for those flows until `flow reindex --repin`; whether scripts should be read live from `flows/<name>.yaml`, with only the states and moves pinned, is the alternative.
