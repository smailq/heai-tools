// operator: an htop-shaped terminal view of what the heai tools are doing:
// the tracker's tasks, the flows that are stuck, and the sessions running.
package main
...
const usage = `operator - what the heai tools are doing, on one screen

  operator                  the screen
  operator --once           the screen, once, to stdout, then exit (every task; --active for the working set)
  operator --json           the tracker, the flows and the sessions as the screen read them, then exit
  ...
  --poll <dur>        the clock for the panes that ask flow and pod (default 5s)
...
a blocked task whose blocker is canceled or missing, or a stuck flow waiting on a flow that ended
without done; 2 no tracker, or bad usage.
`
...
func run(args []string) int {
	...
	if fs.NArg() > 0 {
		fmt.Fprintf(os.Stderr, "operator: unexpected argument %q\n%s", fs.Arg(0), usage)
		return 2
	}
	...
	if *once || *asJSON {
		...
		if snap.Err != nil {
			fmt.Fprintln(os.Stderr, "operator:", snap.Err)
			return 2
		}
		fl, se := ui.LoadFlows(snap.MapPath, now), ui.LoadSessions(snap.MapPath, now)
		if *asJSON {
			return printJSON(snap, fl, se)
		}
		return printOnce(opts, snap, fl, se, *active, *width)
	}
...
func printOnce(opts ui.Options, snap ui.Snapshot, fl flows.Result, se ui.Sessions, active bool, width int) int {
	opts.Plain = true
	opts.NoSplit = true
	if width <= 0 {
	...
	m := ui.New(opts).WithSnapshot(snap).WithFlows(fl).WithSessions(se)
	if active {
		m = m.WithActive()
	}
	// Tall enough for every row: header, pane title, column header, rows, the two other panes' lines, key bar.
	m = m.WithSize(width, m.RowCount()+6)
...
func printJSON(snap ui.Snapshot, fl flows.Result, se ui.Sessions) int {
	out := struct {
		...
		Sessions ui.Sessions       `json:"sessions"`
		Red      bool              `json:"red"`
		At       time.Time         `json:"at"`
	}{snap.Tracker, snap.MapPath, snap.Owners.Owners, snap.Owners.Repos, snap.Owners.Note, snap.Tracker.Counts(), fl, se, snap.Tracker.Red() || fl.Red(), snap.At}
