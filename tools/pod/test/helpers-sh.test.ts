...
test('settled wait: agent get for identity, agent wait for the state; idle is 0, blocked 1, a timeout 2, a vanished agent lost', { skip: !hasJq && 'jq is not installed' }, () => {
  ...
  assert.equal(body.workspace, 'w4')
  assert.equal(body.pane, 'w4:p1')
  assert.equal(body.agent, 'w4')
  assert.equal(body.state, 'idle')
  assert.equal(body.exitCode, 0)
  assert.deepEqual(s.calls().slice(0, 2), ['agent get w4', 'agent wait w4 --until blocked --timeout 5000'])
...
test('settled prompt: agent prompt --wait with the text, and Herdr\'s refusals as states', { skip: !hasJq && 'jq is not installed' }, () => {
  ...
  assert.ok(s.calls().includes('agent prompt w4 do the thing --wait --timeout 120000'))
...
test('settled run: pane wait-output on the sentinel; the matched line carries the exit code', { skip: !hasJq && 'jq is not installed' }, () => {
  ...
  const body = JSON.parse(r.stdout)
  assert.equal(body.workspace, 'w2')
  assert.equal(body.pane, 'w2:p2')
  assert.equal(body.agent, null)
  assert.equal(body.state, 'exited')
  assert.equal(body.exitCode, 1)
  assert.equal(s.calls()[0], 'pane wait-output w2:p2 --regex __heai_exit=[0-9]+ --timeout 15000')
