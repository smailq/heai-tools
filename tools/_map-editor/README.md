...
| `--schema` | `SCHEMA_FILE` | architect's `schemas/architecture.schema.json` | schema to validate against |
...
The validator and the glob dialect are `architect`'s, tested there; the editor's own test spawns the server and exercises the endpoints over it.
