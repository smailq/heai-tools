...
- Edit with: `task-manager new|set --dir tasks`
- Regenerate with: `task-manager build --dir tasks`

## Never hand-edit a task file

Create and change tasks with `task-manager new` and `task-manager set`, not by writing frontmatter yourself.

```sh
task-manager new <slug> --title "..." --body "..." [--status todo] [--priority high] [--territory api]
task-manager set <slug> --status in-progress [--priority high]
task-manager set <slug> --status blocked --note "Blocked by [other-slug](other-slug.md)."
task-manager delete <slug>          # only for a task that should never have been filed
```
...
Use Write or Edit on a task file only to repair one the tool refuses to load, and run `task-manager build` afterwards.
...
- **Recommending work.** Asked what to work on next, read `INDEX.md`, apply the pick-up order (finish `in-progress` first, then `todo`, then triage `backlog` by priority), and give a one-line rationale for each recommendation.
  Read the body of anything you recommend: bodies carry constraints and dependencies the index does not.
