...
Create and change tasks with \`task-manager new\` and \`task-manager set\`, never by writing frontmatter by hand: they validate the change before writing it and regenerate the view after it.
\`task-manager build --check\` exits non-zero when the view is stale, so CI can hold the two in step.
\`task-manager serve --dir <this folder>\` opens the web view, which reads and edits through the same checks.
...
2. **Pick-up protocol:** read \`INDEX.md\`, choose a task (finish \`in-progress\` first, then \`todo\`, then triage \`backlog\`), read its file fully, \`task-manager set <slug> --status in-progress\`, and commit the task-file change alongside the work.
3. **Adding a task:** \`task-manager new <slug> --title "..." --body "..."\` - kebab-case slug, unique, descriptive, and permanent.
   Deleting is \`task-manager delete <slug>\`, which moves the file to \`deleted/\` rather than unlinking it; \`done\` and \`canceled\` are outcomes and stay in \`${ITEMS_DIR}/\`.
