...
/** The schema in this repository, beside the tools; a copied install passes its own. */
...
export function resolveMapPath(given: string | undefined, cwd = process.cwd()): string {
  if (given) return resolve(cwd, given)
  const env = process.env['HEAI_MAP']
...
/** A problem that makes the question unanswerable: the map or schema cannot be read. Exit 2. */
export class LoadError extends Error {}

/** Read and validate a map file. Throws `LoadError` when the file or the schema cannot be read. */
export function loadMap(mapPath: string, schemaPath: string = DEFAULT_SCHEMA): LoadedMap {
  if (!existsSync(schemaPath)) throw new LoadError(`cannot read schema: ${schemaPath}`)
  let text: string
  try {
    text = readFileSync(mapPath, 'utf8')
  } catch {
    throw new LoadError(`cannot read architecture map: ${mapPath}`)
  }
...
export function actorContext(map: ArchitectureMap, actor: string): { own: ContextLayer[]; owned: { territory: string; layers: ContextLayer[] }[]; watched: { territory: string; owner: string | null; layers: ContextLayer[] }[] } {
  ...
  if (!a) throw new LoadError(`no actor named "${actor}" in the map`)
